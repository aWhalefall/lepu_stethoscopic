...
public class RawDatabaseManager {
	...
	// Singleton Pattern
	private static RawDatabaseManager mInstance = null;
	...
	public static void initManager(Context context) {
		if (mInstance == null) {
			mInstance = new RawDatabaseManager(context);
		}
	}
	 ...
	 *          null
	 */
	public static RawDatabaseManager getManager(Context context) {
		if (mInstance != null) {
			return mInstance;
		}
	...
	private RawDatabaseManager(Context context) {
	 ...
	 */
	public SQLiteDatabase getDatabase(int rawResId, String dbfile) {
		...
		SharedPreferences dbs = context.getSharedPreferences(
				RawDatabaseManager.class.toString(), 0);
		...
		if (!flag || !file.exists()) {
			...
			if (!copyRawToFilesystem(rawResId, sfile)) {
	...
	private boolean copyRawToFilesystem(int rawResId, String des) {
		Log.i(tag, "Copy " + rawResId + " to " + des);
		InputStream istream = null;
		OutputStream ostream = null;
		try {
			// AssetManager am = context.getAssets();
			istream = context.getResources().openRawResource(rawResId);
