...
public class RulerWheel extends View {
    ...
    public static final int ITEM_HALF_DIVIDER = 40;
    public static final int ITEM_ONE_DIVIDER = 10;
    ...
    private int mNarrowTimes = 1;  //数值缩小倍数(默认单位为1)
     ...
     *
     */
    public void initView(int defaultValue, int maxValue, int model) {
        switch (model) {
            case MOD_TYPE_HALF:
    ...
    //默认为小刻度类型
    public void initView(int defaultValue, int maxValue) {
        mModType = MOD_TYPE_ONE;
        mLineDivider = ITEM_ONE_DIVIDER;
        mValue = defaultValue;
        mMaxValue = maxValue;
    ...
    //添加缩小倍数初始化方法
    public void initViewNarrow(int defaultValue, int maxValue,int narrowTimes, boolean isMove) {
        mModType = MOD_TYPE_ONE;
        mLineDivider = ITEM_ONE_DIVIDER;
        mValue = defaultValue;
        mMaxValue = maxValue;
        mNarrowTimes = narrowTimes;
        mIsMove = isMove;
        invalidate();
        mLastX = 0;
        mMove = 0;
                    ...
                    if (mValue + i <= mMaxValue) {
                        switch (mModType) {
                            case MOD_TYPE_HALF:
                                ...
                                break;
                            case MOD_TYPE_ONE:
                                canvas.drawText(String.valueOf((mValue + i) / mNarrowTimes), xPosition - (textWidth * numSize / 2), getHeight() - textWidth, textPaint);
                                break;
    ...
    private void drawMiddleLine(Canvas canvas) {
        ...
        int gap = 12;
        int indexWidth = 6; //中间刻度线的宽度
        int indexTitleWidth = 24;
        int indexTitleHight = 8; //俩头的高
        int shadow = 6;
    ...
    private void notifyValueChange() {
        if (null != mListener) {
            if (mModType == MOD_TYPE_ONE) {
                mListener.onValueChange((float)mValue / mNarrowTimes);
            }
